...
final class FLBuilderUserTemplatesAdminMenu {
	...
	static public function submenu_file( $submenu_file, $parent_file )
	{
		...
		else if ( isset( $_GET['fl-builder-template-type'] ) && $list_url == $parent_file ) {
			$type = sanitize_text_field( $_GET['fl-builder-template-type'] );
			$submenu_file = admin_url( $parent_file . '&fl-builder-template-type=' . $type );
		}
		else if ( 'post.php' == $pagenow && 'fl-builder-template' == $screen->post_type  ) {
		...
		else if ( 'edit-tags.php' == $pagenow && 'fl-builder-template-category' == $screen->taxonomy ) {
			$submenu_file = admin_url( 'edit-tags.php?taxonomy=fl-builder-template-category&post_type=fl-builder-template' );
		}
		else if ( 'term.php' == $pagenow && 'fl-builder-template-category' == $screen->taxonomy ) {
			$submenu_file = admin_url( 'edit-tags.php?taxonomy=fl-builder-template-category&post_type=fl-builder-template' );
		}
